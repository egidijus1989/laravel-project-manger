...
class DashboardController extends Controller
{
    public function index(){
        $user = auth()->user();
        $totalPendingTasks = Task::query()->where("status", 'pending')
        ->count();
        $myPendingTasks = Task::query()->where("status", 'pending')
        ->where('assigned_user_id', $user->id)->count();

        $totalInProgressTasks = Task::query()->where("status", 'in_progress')
        ->count();
        $myInProgressTasks = Task::query()->where("status", 'in_progress')
        ->where('assigned_user_id', $user->id)->count();

        $totalCompleteTasks = Task::query()->where("status", 'completed')
        ->count();
    ...
    "totalInProgressTasks", "myInProgressTasks", "totalCompleteTasks",
    "myCompleteTasks", "activeTasks"));
    }
}
